...
DEFAULT_CONFIG = {
    ...
    'MODEL': 4,
    # 1: random block, 2: half, 3: external, 4: (external, random block), 5: (external, random block, half)
    'MASK': 3,
    'EDGE': 1,                      # 1: canny, 2: external
    # 0: no non-max-suppression, 1: applies non-max-suppression on the external edges by multiplying by Canny
    'NMS': 1,
    'SEED': 10,                     # random seed
    ...
    # standard deviation of the Gaussian filter used in Canny edge detector (0: random, -1: no edge)
    'SIGMA': 2,
