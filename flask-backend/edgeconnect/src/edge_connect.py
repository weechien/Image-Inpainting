...
from edgeconnect.src.utils import output_align
    ...
    def __init__(self, config):
        ...
        if self.config.MODE == 2:
            ...
            self.train_dataset = Dataset(
                config, config.TRAIN_FLIST, config.TRAIN_EDGE_FLIST, config.TRAIN_MASK_FLIST, augment=True, training=True)
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                if model == 1:
                    ...
                    # metrics
                    precision, recall = self.edgeacc(
                        edges * masks, outputs * masks)
                ...
                # joint model
                    ...
                    outputs_merged = (i_outputs * masks) + \
                        (images * (1 - masks))
    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            if self.debug:
                ...
                imsave(edges, os.path.join(
                    self.results_path, fname + '_edge.' + fext))
                imsave(masked, os.path.join(
                    self.results_path, fname + '_masked.' + fext))
